...
router.get('/', function (req, res, next) {
  if (isNaN(req.session.user_id)) {
    ...
    if (searchWord) {
      var sql = "select count(*) as cnt from (SELECT * FROM note where p_id = ? and article_subject LIKE ? ORDER BY time DESC) as total  group by article_subject ORDER BY time DESC";
      pool.query(sql, [p_id, '%' + searchWord + '%'], function (err, total) {
        if (err) throw err;
        // var totalLine = results1[0].cnt;
        // var totalPage = Math.ceil(totalLine / linePerPage);
        sql2 = "SELECT * FROM (SELECT * FROM note where p_id = ? and article_subject LIKE ? ORDER BY time DESC) as total   GROUP BY  article_subject  ORDER BY time DESC "
        pool.query(sql2, [p_id, '%' + searchWord + '%'], function (err, groups) {


          res.render('group/project_Note', { pageNo: pageNo, p_id: p_id, searchWord: searchWord, groups: groups, total: total });
  ...
  pool.query("select count(*) as cnt from note where content LIKE ?", [search_Word], function (err, results1) {
    ...
    pool.query('select * from note where p_id = ? ', [p_id], function (err, info) {
      ...
      pool.query('select * from note where content LIKE ? order by time desc limit ?, ?', [search_Word, (pageNo - 1) * linePerPage, linePerPage], function (err, results) {
        if (err) throw err;
        res.render('group/project_Note', { data: results, info: info, pageNo: pageNo, totalLine: totalLine, totalPage: totalPage, linePerPage: linePerPage, p_id: p_id, searchWord: searchWord });
      })
    })
  })
})
